...
from .tokenization import convert_to_unicode, FullTokenizer, printable_text
...
class SimProcessor(DataProcessor):
    def get_train_examples(self, data_dir):
        ...
        for index, train in enumerate(train_df.values):
            guid = 'train-%d' % index
            text_a = convert_to_unicode(str(train[0]))
            text_b = convert_to_unicode(str(train[1]))
            label = str(train[2])
    ...
    def get_dev_examples(self, data_dir):
        ...
        for index, dev in enumerate(dev_df.values):
            guid = 'test-%d' % index
            text_a = convert_to_unicode(str(dev[0]))
            text_b = convert_to_unicode(str(dev[1]))
            label = str(dev[2])
    ...
    def get_test_examples(self, data_dir):
        ...
        for index, test in enumerate(test_df.values):
            guid = 'test-%d' % index
            text_a = convert_to_unicode(str(test[0]))
            text_b = convert_to_unicode(str(test[1]))
            label = str(test[2])
    ...
    def get_sentence_examples(self, questions):
        for index, data in enumerate(questions):
            guid = 'test-%d' % index
            text_a = convert_to_unicode(str(data[0]))
            text_b = convert_to_unicode(str(data[1]))
            label = str(0)
...
class BertSim:
    ...
    def __init__(self, batch_size=args.batch_size):
        ...
        self.tokenizer = FullTokenizer(vocab_file=args.vocab_file, do_lower_case=True)
    ...
    def get_estimator(self):
        ...
        bert_config = BertConfig.from_json_file(args.config_name)
