...
from .vectorize import create_embedding, sentence_process
...
def create_valid_data():
    if os.path.exists(valid_file) == False:
        with open(test_file, 'r', encoding='utf-8', errors='ignore') as fin:
            lines = sorted(fin.readlines(), key=lambda v: random.random())
            sents = len(lines)
...
def create_data_file(in_file, out_lable_file, out_text_file):
    ...
    with open(in_file, 'r', encoding='utf-8', errors='ignore') as fin:
        ...
        for line in lines:
            ...
            lable_count[np.argmax(lables)] += 1
    # print(lens_list)
...
lable_count = np.zeros((8))
lable_weight = np.zeros((8))
...
def preprocess():
    ...
    print('8 Class: ', lable_count)
    lable_weight = lable_count / np.sum(lable_count)
    print('8 Class Weight: ', lable_weight)
    lable_weight = np.array([1 / c for c in lable_weight])
    print('8 Class 1 / Weight: ', lable_weight)
